...
window.Updater = (function () {
  ...
  function banner(remote) {
    ...
    bar.innerHTML = "<b>Update available</b> — v" + window.CS_VERSION + " vs GitHub v" + remote.version + ". Close the game and run <code>update.bat</code>. <button type=\"button\" id=\"upd-dismiss\">Later</button>";
    document.body.insertBefore(bar, document.body.firstChild);
    document.getElementById("upd-dismiss").onclick = function () { bar.parentNode.removeChild(bar); };
  }
  function check() {
    var req = new XMLHttpRequest(); req.open("GET", REMOTE, true); req.timeout = 6000;
    req.onreadystatechange = function () {
      if (req.readyState !== 4 || req.status < 200 || req.status >= 300) return;
      try { var remote = JSON.parse(req.responseText); if (cmp(window.CS_VERSION, remote.version) < 0) banner(remote); } catch (e) {}
    };
